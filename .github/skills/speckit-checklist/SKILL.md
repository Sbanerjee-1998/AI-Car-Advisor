...
- For each executable hook, output the following based on its `optional` flag:
  ...
  - **Mandatory hook** (`optional: false`):
    ...
    **Automatic Pre-Hook**: {extension}
    ...
    ```
    After emitting the block above you MUST actually invoke the hook and wait for it to finish before continuing to the Execution Steps. Run it the same way you would run the command yourself in this agent/session (the invocation may differ from the literal `{command}` id shown above, e.g. a skills-mode agent runs it as `/skill:speckit-...` or `$speckit-...`). Emitting the block alone does not run the hook.
...
6. **Generate checklist** - Use TEMPLATE_CONTENT as the structural template and create "Unit Tests for Requirements":
   ...
   **HOW TO WRITE CHECKLIST ITEMS - "Unit Tests for English"**:
   ...
   - "Is the number and layout of UI elements explicitly specified?" [Completeness, Spec §FR-1]
   ...
   **EXAMPLES BY QUALITY DIMENSION**:
   ...
   - "Can 'balanced visual weight' be objectively verified? [Acceptance Criteria, Spec §FR-2]"
   ...
   **🚫 ABSOLUTELY PROHIBITED** - These make the checklist an implementation test, not a requirements test:
...
**Important**: Each `/speckit-checklist` command invocation uses a short, descriptive checklist filename and either creates a new file or appends to an existing file. This allows:

- Multiple checklists of different types (e.g., `ux.md`, `test.md`, `security.md`)
- Simple, memorable filenames that indicate checklist purpose
- Easy identification in the `checklists/` folder
...
- "Are error response formats specified for all API failure scenarios? [Completeness]"
